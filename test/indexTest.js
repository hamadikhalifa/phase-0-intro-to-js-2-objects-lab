...
describe("employee", function () {
  it("has a name and street address", function () {
    expect(employee.name).to.exist;
    expect(employee.streetAddress).to.exist;
  });
  ...
  describe("updateEmployeeWithKeyAndValue(employee, key, value)", function () {
    ...
    it("does not modify the original employee, but rather returns a clone with the new data", function () {
      ...
      expect(employee["streetAddress"]).to.equal("11 Broadway"); // Fix expectation
    });
  });
